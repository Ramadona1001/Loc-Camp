...
@section('content')
    ...
    <h2 class="text-center mb-5">{{ $user->name }}</h2>
    <div class="row">
                    ...
                    @foreach ($permissions as $permission)
                        @if (explode('_',$permission->name)[1] == $permissionName)
                            <div class="row">
                                ...
                                <input type="checkbox" class="myinput large custom" name="" id="{{ $permission->id }}" value="{{ $permission->id }}" name="permissions[]" checked style="margin-left: 20px;margin-right:20px;">
                                @else
                                <input type="checkbox" class="myinput large custom" name="" id="{{ $permission->id }}" value="{{ $permission->id }}" name="permissions[]" style="margin-left: 20px;margin-right:20px;">
                                @endif
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('input[type="checkbox"]').click(function(){
            ...
            var roleId = '{{ $user->id }}';
            var assignPermissionUrl = '{{ route("assign_premissions_users",["id"=>"#id"]) }}';
